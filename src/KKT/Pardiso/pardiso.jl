...
PardisoSQD backend for solving linear systems.

See [`PardisoSQD`](@ref) for further details.
"""
struct Backend <: AbstractKKTBackend end

"""
    PardisoSQD{T,S<:AbstractKKTSystem}

Pardiso-based KKT solver.
...
* To solve the augmented system with Pardiso's ``LDL^{T}`` factorization:
```julia
set_parameter(tlp_model, "KKT_Backend", Tulip.KKT.PardisoSQD.Backend())
set_parameter(tlp_model, "KKT_System", Tulip.KKT.K2())
```

# Remark

You need to obain a licence for the Panua Pardiso to use this solver.
"""
mutable struct PardisoSQD{S} <: AbstractKKTSolver{Float64}
...
function setup(A::SparseMatrixCSC{Float64, Int}, ::K2, ::Backend)
    ...
    K_pardiso = Pardiso.get_matrix(ps, K, :N)
    @assert istril(K_pardiso)
    ...
    Pardiso.pardiso(ps, K_pardiso, ones(m+n))

    return PardisoSQD{K2}(m, n, A, θ, regP, regD, K_pardiso, ξ, ps)

end

backend(::PardisoSQD) = "Pardiso"
linear_system(::PardisoSQD) = "Augmented system (K2)"

function update!(kkt::PardisoSQD, θ, regP, regD)
    m, n = kkt.m, kkt.n
...
function solve!(dx, dy, kkt::PardisoSQD, ξp, ξd)
    m, n = kkt.m, kkt.n
    ...
    Pardiso.pardiso(kkt.ps, d, kkt.K, ξ)
